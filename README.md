...
Phase 0 of the plan: validate demand with two waitlists (`demand` = people who hit limits, `supply` = people with idle GPUs) before building the gateway.
...
2. **Cloudflare Web Analytics** — add the site in Cloudflare (Web Analytics → manual setup) and paste the beacon `<script>` snippet just before `</body>`. Use the **manual JS snippet**, not the nameserver/proxy option — proxying would overwrite the A records GitHub Pages needs if a custom domain is added later.
